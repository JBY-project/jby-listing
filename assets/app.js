...
function onScroll(){
  ...
  if(topbarWrap) topbarWrap.classList.toggle('stuck', y > 40);
  if(bar){
...
/* request info — from the sticky bar's Contact dealer */
var rqModal = simpleModal({
  id: 'rq', openSelector: '[data-open-request]', closeAttr: 'data-rq-close',
  submitId: 'rq-submit', errId: 'rq-err', doneMsgId: 'rq-done-msg',
  validate: function(){
    var miss = [];
    if(!document.getElementById('rq-name').value.trim()) miss.push('your full name');
    if(!EMAIL_RE.test(document.getElementById('rq-email').value.trim())) miss.push('a valid email');
    if(!document.getElementById('rq-message').value.trim()) miss.push('a message');
    return miss;
  },
  message: function(){
    return 'Thanks \u2014 your message about the ' + VESSEL + ' is with the dealer. ' +
           'They\u2019ll reply at ' + document.getElementById('rq-email').value.trim() + ' shortly.';
  }
});
var rqPhoneBtn = document.getElementById('rq-phone-btn');
if(rqPhoneBtn) rqPhoneBtn.addEventListener('click', function(){
  rqPhoneBtn.textContent = '+1 (888) 693-8099';
});
...
function calc(){
  ...
  var amount = '$' + fmt(m || 0);
  if(elMonthly) elMonthly.textContent = amount;
  var mAmount = document.getElementById('m-monthly');
  if(mAmount) mAmount.textContent = amount;
}
...
/* ---------- mobile sticky bar: swap to the loan summary over that section ---------- */
(function(){
  var loan = document.getElementById('loan');
  if(!loan || !('IntersectionObserver' in window)) return;
  var phone = window.matchMedia('(max-width:820px)');
  var inView = false;
  var apply = function(){
    document.body.classList.toggle('loan-view', inView && phone.matches);
  };
  new IntersectionObserver(function(entries){
    inView = entries[0].isIntersecting;
    apply();
  }, {rootMargin:'-20% 0px -30% 0px', threshold:0}).observe(loan);
  if(phone.addEventListener) phone.addEventListener('change', apply);
  var applyBtn = document.getElementById('m-apply');
  if(applyBtn) applyBtn.addEventListener('click', function(){
    var real = document.querySelector('.loan-out .btn-navy');
    if(real) real.click();
  });
})();
...
function show(i){
  ...
  lbxCap.textContent = (idx + 1) + ' / ' + figs.length;
}
...
document.addEventListener('keydown', function(e){
  if(e.key === 'Escape' && rqModal && rqModal.el.classList.contains('open')){ rqModal.close(); return; }
