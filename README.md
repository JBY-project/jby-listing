...
| `#st` | Schedule a tour (sidebar, or the tour prompt on phones) | mode, live calendar, slot picker, form, confirmation |
...
| `#rq` | Contact dealer, in the mobile sticky bar | message prefilled with the vessel and asking price |

All eight close on Esc, veil click and the ✕, reset when reopened, and end
on the same check-circle confirmation. Footers carry a single action —
dismissal is the ✕ or the veil.

---

## Mobile (below 820px)

- **Sticky action bar** — Contact dealer (opens `#rq`) and Call
  (`tel:` the toll-free line). Over the loan section it swaps for a live
  Monthly payment summary with Apply now, driven by an
  IntersectionObserver on `#loan` toggling `.loan-view` on `<body>`.
- The header and the Back / Share row are both pinned; the header holds
  one height in either state so the row can stick under it.
- The sidebar card and the loan calculator's payment panel are hidden —
  the sticky bar and the modals carry both. A tour prompt above the
  description keeps Schedule a tour reachable.
- Spec tabs scroll sideways on one row; the spec grid is two columns;
  loan terms are one row of three with short labels.
- Two section headings still wrap: they carry the vessel name and cannot
  fit one line at a sensible size.
...
1. **Forms don't submit.** Every modal and the sidebar form validate client
   side, then show the confirmation state. No endpoint is called. The
   Call button and Show phone number both use the toll-free line from the
   footer.
2. **Listing data is hard-coded** in `index.html` (title, price, spec strip)
   and in `SPECS` / `RS_VESSELS` / `answers` in `app.js`.
3. **The `#rq` message is prefilled** from hard-coded vessel and price
   strings in `index.html`.
4. **Tour availability is generated**, not fetched — see `slotsFor()` in
...
5. **Price Value Factors** still carry the Figma placeholders (`[YEAR]`,
   `[MAKE]`, `[MODEL]`…).
6. **3D Tour** is a still image with a Matterport badge, not an embed.
7. **Links** in the header, footer and Other listings point at `#`.
8. Loan calculator figures are indicative only.
